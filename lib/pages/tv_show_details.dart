...
class _TvShowDetailsState extends State<TvShowDetails> {
  ...
  Widget build(BuildContext context) {
    ...
    void onRemoveShow(int id) => StoreProvider.dispatch(context, RemoveTvShowAction(id));
    void onToggleArchive(int id, bool archive) => StoreProvider.dispatch(context, ToggleArchiveTvShowAction(id, archive));
    void onToggleEpisodeWatched(Episode episode, bool watched) =>
        StoreProvider.dispatch(context, ToggleEpisodeWatchedAction(widget.tvShowId, episode, watched));
    void onToggleWidget(int id, bool showInWidget) => StoreProvider.dispatch(context, ToggleShowInWidgetAction(id, showInWidget));
            ...
            PopupMenuButton<int>(
              ...
              itemBuilder: (BuildContext context) => <PopupMenuItem<int>>[
                  ...
                  PopupMenuItem<int>(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(tvShowLocal.showInWidget == true ? 'Remove from widget' : 'Add to widget'),
                      ],
                    ),
                  ),
                PopupMenuItem<int>(
                ...
                PopupMenuItem<int>(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.delete_outline, size: 20, color: Theme.of(context).colorScheme.onSurfaceVariant),
                      const SizedBox(width: 12),
                      const Text('Delete series'),
                    ],
                  ),
                ),
              ],
                      ...
                      child: Column(
                        children: [
                          for (int i = 0; i < seasons.length; i++) ...[
                            if (i > 0) Divider(),
                            SeasonListTile(key: ValueKey(seasons[i].id ?? seasons[i].seasonNumber), tvShowId: widget.tvShowId, season: seasons[i]),
                          ],
                        ],
                      ),
                    ),
                      ...
                      child: Column(
                        children: [
                          for (int i = 0; i < specials.length; i++) ...[
                            if (i > 0) Divider(),
                            SeasonListTile(key: ValueKey(specials[i].id ?? specials[i].seasonNumber), tvShowId: widget.tvShowId, season: specials[i]),
                          ],
                        ],
                      ),
                    ),
